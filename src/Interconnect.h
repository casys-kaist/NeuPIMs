...
class Interconnect {
   public:
    ...
    virtual std::pair<MemoryAccess *, MemoryAccess *> get_candidates(uint32_t nid) = 0;

    void log();
    void update_stat(MemoryAccess mem_access, uint64_t ch_idx);
    inline cycle_type get_core_cycle();

    std::vector<std::list<MemoryAccess *>> _out_buffers;
...
class SimpleInterconnect : public Interconnect {
   public:
    ...
    virtual std::pair<MemoryAccess *, MemoryAccess *> get_candidates(uint32_t nid) override;
