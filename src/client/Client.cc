...
void Client::cycle() {
    ...
    while (!_touch) {
        ...
        // uint32_t channel = input_output_size.second;  // input_output_size.second;  // 1;
        ...
        _need_wait_cycles = _distribution(_gen);

        // spdlog::info("Client Request Departure!! now:{} next wait: {}", _cycles,
        // _need_wait_cycles); spdlog::info("Request #{}, input size:{}, output size:{}", rid,
