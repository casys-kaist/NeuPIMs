...
class CPU {
   ...
   protected:
    ...
    uint64_t MakePHeaderPacket(int channel, int row, bool for_gwrite, int num_comps,
                               int num_readres);
};
...
class RandomCPU : public CPU {
   ...
   private:
    ...
    std::mt19937_64 gen;  // std::mt19937_64 => 64-bit random number generator
    bool get_next_ = true;
    int loop_cnt_ = 1;
    bool gemv_turn_ = false;
    int num_comps_;
    int num_readres_;
};
